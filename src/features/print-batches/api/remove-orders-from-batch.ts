...
import type { PrintBatch, RemoveOrdersFromBatchParams } from '../types';
...
export const useRemoveOrdersFromBatch = createMutation({
  mutationFn: ({
    batchId,
    orderIds,
  }: RemoveOrdersFromBatchParams): Promise<PrintBatch> =>
    api.delete(`admin/print-batches/${batchId}/orders`, { data: { orderIds } }),
  use: [
    invalidateQueries([
